"""
Plot functions for easy and fast visualiztaion of images and regsitration results
"""
...
def plot_antsimgs(ants_img, figpath, title="", vmin=0, vmax=500):
    ...
    if figpath:
        ...
        im = ax[2].imshow(ants_img[:,:, half_size[2],], cmap='gray', vmin=vmin, vmax=vmax)
...
def plot_reg(moving, fixed, warped, figpath, title="", loc=0, vmin=0, vmax=1.5):
    ...
    if loc >= len(moving.shape):
        raise ValueError(
            f"loc {loc} is not allowed, should be less than are {len(moving.shape)}"
    ) 
...
def plot_reg_before_after(moving, fixed, warped, figpath, title="", loc=0, vmin=0, vmax=1.5):
    """
    if moving and fixed images have same dimension, 
    plot registration results: moving, fixed, deformed,
    overlay and difference images before and after registration
    ...
    # combine deformed and fixed images to an RGB image
    # green: fixed image, magenta: moving/moved image 
    overlay0 = np.stack( (moving, fixed, moving), axis=2 ) # before registration
    overlay1 = np.stack( (warped, fixed, warped), axis=2 ) # after registration
    ...
    ax[1].imshow(fixed, cmap='gray', vmin=vmin, vmax=vmax)
    ax[2].imshow(warped, cmap='gray', vmin=vmin, vmax=vmax)
    ax[3].imshow(overlay0)
    ax[4].imshow(diff0, cmap='gray', vmin=-(vmax), vmax=vmax)
    ax[5].imshow(overlay1)
    ax[6].imshow(diff1, cmap='gray', vmin=-(vmax), vmax=vmax)

    ax[0].set_title("Moving", fontsize=fontsize)
    ax[1].set_title("Fixed", fontsize=fontsize)
    ax[2].set_title("Deformed", fontsize=fontsize)
    ax[3].set_title("Before registration\nDeformed Overlay Fixed", fontsize=fontsize-2)
    ax[4].set_title("Before registration\nFixed - Deformed", fontsize=fontsize-2)
    ax[5].set_title("After registration\nDeformed Overlay Fixed", fontsize=fontsize-2)
    ax[6].set_title("After registration\nFixed - Deformed", fontsize=fontsize-2)
